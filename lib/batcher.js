...
//   batchSize:     How many to batch together, default is 200.
...
Batcher.prototype._processQueue = function _processQueue() {
  ...
  while (this._paused === false && this._messages.size() !== 0 &&
        (this._messages.size() >= this._batchSize || this._flushRequested)) {

    var payload = this._messages.dequeueMultiple(this._batchSize);
...
Batcher.prototype.write = function write(obj) {
  if (this._messages.size() < this._batchOverflow) {
    ...
    if (this._batchOverflow === -1) {
      this.emit('error', new Error('Trying to write() a close()-ed batcher'));
    } else {
      this.emit('error', new Error('Too many undelivered messages'));
    }
  }
...
Batcher.prototype.setBatchSize = function setBatchSize(size) {
  ...
  if (this._messages.size() >= this._batchSize) {
    this._processQueue();
  }
};
